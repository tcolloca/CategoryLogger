...
public class CategoryLogger {
  ...
  private final Map<String, List<String>> categoryOuts =
      Collections.synchronizedMap(new HashMap<>());
  private final Map<String, List<LogListener>> categoryListeners =
      Collections.synchronizedMap(new HashMap<>());
      ...
      for (String parent : closure) {
        if (categoryOuts.containsKey(parent)) {
          for (String outFilePath : categoryOuts.get(parent)) {
            try {
              PrintStream fileOut =
                  new PrintStream(new FileOutputStream(Paths.get(outFilePath).toFile(), true));
   ...
   */
  public void logTime(String category, String message, long time, TimeUnits srcUnits,
      TimeUnits dstUnits) {
   ...
   * Logs time with the corresponding message, under the category. It should contain %f.
   */
  public void logTime(String category, String message, double time, TimeUnits srcUnits,
      TimeUnits dstUnits) {
    if (message == null) {
      throw new IllegalArgumentException("message is null.");
    }
